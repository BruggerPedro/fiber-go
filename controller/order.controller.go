...
func SubTotalOrderController(c *fiber.Ctx) error {
	//Token authenticate
	headerToken := c.Get("Authorization")
	err := middleware.Auth(headerToken)
	if err != nil {
...
func CheckOrderController(c *fiber.Ctx) error {
...
func FindOrderController(c *fiber.Ctx) error {
	id := c.Params("id")
	var order model.Order
...
func ListOrderController(c *fiber.Ctx) error {
	var count int64
	var order []model.Order
...
func DownloadOrderController(c *fiber.Ctx) error {
	param := c.Params("orderId")
	var order model.Order
